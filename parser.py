...
# We use a regex here to clean characters and keep only numerals
cities_best = []
cities_set = []
cities_tups = []
cities_dict = {}
...
def read_tsp_data(tsp_name):
    ...
    with open(tsp_name) as f:
        ...
        cleaned = [x.strip() for x in content if x != ""]
        return cleaned
...
def get_cities(incominglist, dimension):
...
def create_cities_dict(city_tuples):
    return zip((range(1, len(city_tuples) + 1)), city_tuples)
...
def plot_cities(city_tuples):
    plt.clf()
    plt.scatter(*zip(*city_tuples))
    plt.plot(*zip(*city_tuples))
    plt.show()
...
def produce_final(file="a280.tsp"):
